...
const AdminDashboard = () => {
    ...
    const [formData, setFormData] = useState({
        title: '', description: '', date: '', location: '', category: '', totalAmount: '', productPrice: '', image: ''
    });
    ...
    const handleCreateEvent = async (e) => {
        ...
        try {
            ...
            setFormData({ title: '', description: '', date: '', location: '', category: '', totalAmount: '', productPrice: '', image: '' });
            fetchData();
        } catch (error) {
        ...
        <div className="max-w-7xl mx-auto">
            <div className="bg-black text-white rounded-2xl p-6 sm:p-8 mb-8 shadow-lg flex flex-col md:flex-row justify-between items-center gap-6 text-center md:text-left">
                <div>
                    ...
                    <p className="text-gray-300">Manage Product and manually confirm bookings.</p>
                </div>
                <button
            ...
            {showEventForm && (
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 mb-8 animation-slideDown">
                    ...
                    <form onSubmit={handleCreateEvent} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <input required type="number" placeholder="Total Seats" className="border px-4 py-3 rounded-lg focus:ring-2 focus:ring-gray-700 outline-none transition" value={formData.totalSeats} onChange={e => setFormData({ ...formData, totalAmount: e.target.value })} />
                        <input required type="number" placeholder="Ticket Price (0 for free)" className="border px-4 py-3 rounded-lg focus:ring-2 focus:ring-gray-700 outline-none transition" value={formData.ticketPrice} onChange={e => setFormData({ ...formData, productPricePrice: e.target.value })} />
                        ...
                        <button type="submit" className="md:col-span-2 bg-gray-900 text-white font-bold py-3 mt-2 rounded-lg hover:bg-black transition shadow-md">Publish Product</button>
                    </form>
                </div>
            )}
                ...
                <div className="flex flex-col">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                        <ul className="divide-y divide-gray-100 max-h-[600px] overflow-y-auto">
                            {events.length === 0 ? <li className="p-6 text-gray-500 text-center">No Product created yet.</li> :
                                events.map(event => (
                                    <li key={event._id} className="p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:bg-gray-50 transition border-b border-gray-100 last:border-0">
                                        <div>
                                            ...
                                            <div className="flex flex-wrap items-center gap-3 text-sm text-gray-500">
                                                ...
                                                <span className="flex items-center gap-1 font-medium"><div className={`w-2 h-2 rounded-full ${event.availableAmount > 0 ? 'bg-green-500' : 'bg-red-500'}`}></div> {event.availableAmount}/{event.totalAmount} Product</span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="bg-gray-50 rounded-lg p-3 mb-3 border border-gray-100 text-sm">
                                            ...
                                            {booking.eventId && (
                                                <p className="text-gray-700 flex items-center gap-2 mt-2 pt-2 border-t border-gray-200">
                                                    <span className="font-bold w-16 text-gray-500 uppercase text-xs">Product</span>
                                                    <span className={`font-bold ${booking.eventId.availableAmount > 0 ? 'text-green-600' : 'text-red-500'}`}>{booking.eventId.availableSeats}</span> remaining of {booking.eventId.totalSeats}
                                                </p>
                                            )}
                                        </div>
